...
void setup() {
  ...
  SerialBT.begin("Autonomus System");
    ...
    if (targetLat == 0 && targetLong == 0) {
      ...
      if (distanceToTarget >= 5) {
        ...
        if (i == size) {
          stopp();
          delay(100000);
          sendLogOverBluetooth();  // <-- Send data when all points are completed
        }
        nextTarget();
      }
    }
...
void readGPS() {
  while (gpsSerial.available()) {
    ...
    if (gps.location.isUpdated()) {
      ...
      // Log GPS point every 5 seconds
      if (millis() - lastLogTime > logInterval) {
        GpsLogEntry entry = { currLat, currLong };
        gpsLog.push_back(entry);
        lastLogTime = millis();
      }
    }
  }
...
// --- New Function to send GPS log over Bluetooth ---
void sendLogOverBluetooth() {
  SerialBT.println("\n--- GPS Path Log (CSV) ---");
  SerialBT.println("Latitude,Longitude");
  for (GpsLogEntry entry : gpsLog) {
    SerialBT.print(entry.lat, 6);
    SerialBT.print(",");
    SerialBT.println(entry.lng, 6);
  }
  SerialBT.println("--- End of Log ---");
}
// ---------------------------------------------------
